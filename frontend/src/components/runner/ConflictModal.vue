...
<template>
  <Teleport to="body">
    <div v-if="show" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-opacity">
      <FocusTrap :active="show" initialFocus="#nuke-btn">
        <div 
          class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-amber-200 dark:border-amber-900/50 animate-in"
          role="dialog"
          aria-modal="true"
        ...
        <div class="p-8 flex flex-col items-center text-center">
          ...
          <h3 id="conflict-modal-title" class="text-xl font-bold text-gray-900 dark:text-white mb-3 tracking-tight">Port Conflict Detected</h3>
          <p id="conflict-modal-desc" class="text-sm text-gray-500 dark:text-gray-400 mb-8 leading-relaxed px-4">
